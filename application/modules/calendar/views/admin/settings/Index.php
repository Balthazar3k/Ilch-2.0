...
<form class="form-horizontal" method="POST" action="<?php echo $this->getUrl(array('action' => 'index')); ?>">
    ...
    <fieldset>
        <legend><?=$this->getTrans('eventCalendar');?></legend>
        <div class="form-group">
            <label for="eventConfig[event_admin_rowsperpage]" class="col-lg-3 control-label">
                <?php echo $this->getTrans('event_calendar_size'); ?>:
            </label>
            <div class="col-lg-4">
                <input class="form-control"
                       id=""
                       type="text"
                       name="eventConfig[event_calendar_size]"
                       placeholder="1-100"
                       value="<?=$config->get('event_calendar_size');?>" />
            </div>
        </div>
    </fieldset>
    <br />
            ...
            <div class="status-box" style="<?=$statusArray['style']?>">
                ...
                <?php foreach($statusArray as $input => $value ): ?>
                    <div class="form-group">
                        ...
                        <div class="col-lg-4">
                        <?php if( $input == 'status' || $input == 'regist' ): ?>
                            <input class="form-control"
                                   type="text"
                                   name="statusConfig[<?=$id;?>][<?=$input;?>]"
                                   value="<?=$value;?>"
                                   <?=( ($input == 'status' || $input == 'regist') ? 'readonly="readonly"' : '');?>
                                   />
                        <?php else: ?>
