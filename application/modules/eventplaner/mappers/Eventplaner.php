...
class Eventplaner extends \Ilch\Mapper
{
    ...
    public function save(EventModel $eventplaner)
    {
        ...
        if($eventId) {
            ...
            $this->db()->insert('ep_events')
                ->fields($fields)
                ->execute();
        }
    }

    public function changeStatus(EventModel $status)
    {
        return $this->db()->update('ep_events')
            ->fields(array('status' => $status->getStatus() ))
            ->where(array('id' => $status->getId()))
            ->execute();
    }
}
?>